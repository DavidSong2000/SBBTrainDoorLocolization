...
#afterwards do indirect 2d-3d matching, we have code that does our direct 2d-3d matching approach that we call sold3d.
...
def sample_3dline_points(line_seg, num_samples=5, min_dist_pts=0.01): #sample max 5 points on lines are at least 10 cm apart
    """
...
def line_localization(cfg, imagecols_db, imagecols_query, point_corresp, linemap_db, retrieval, results_path, img_name_dict=None, logger=None):
    ...
    runs line matching using our custom direct 2d-3d line matching sold3d
    ...
    if cfg['localization']['2d_matcher'] != 'epipolar':
        ...
        if True:
            ...
            #to visualize sample points along line we save images to later put the sample points on it and then save these visualizations
            drawn_imgs = {}
            for img_id in imagecols_db.get_img_ids():
                img_path = imagecols_db.image_name(img_id)
                img = cv2.imread(img_path)
                drawn_imgs[img_id] = img

            #per track, i.e. per 3d line, we compute the 3d line descriptor based on our own approach named sold3d
            for track_id in tqdm(range(n_tracks)):
                track = linemap_db[track_id]
                l3d = track.line 
                l3d_line_seg = np.array([[l3d.start, l3d.end]]) #3d line segment
                ...
                for idx in range(track.count_lines()): #go over all images that contain lines that match to the 3d line
                    ...
                    img = camview2.read_image(set_gray=False)
                    _, full_desc, _, _ = extractor.detector.detect(img) #inspired by sold2.py, extract()
                    #full_desc contains sold2 descriptors per image pixel
                    ...
                    for sample_id in range(l3d_valid_points.shape[1]):
                        if l3d_valid_points[0, sample_id]:
                            ...
                            #for visualization
                            drawn_imgs[img_id] = cv2.circle(drawn_imgs[img_id], proj_p2d.astype(int), radius=1, color=[0, 0, 255])

                    full_desc = torch.tensor(full_desc, dtype=torch.float, device=extractor.detector.device)
                    l2d_line_points = torch.tensor(l2d_line_points.reshape(-1, 2),
                                                dtype=torch.float, device=extractor.detector.device)
                    # Extract the sold 2 descriptors for each 2d point
                    grid_size = extractor.detector.line_matcher.line_matcher.grid_size
                    img_size1 = (full_desc.shape[2] * grid_size, full_desc.shape[3] * grid_size)
                    grid = keypoints_to_grid(l2d_line_points, img_size1)
                    desc_sampled = F.normalize(F.grid_sample(full_desc, grid)[0, :, :, 0], dim=0)
                    if len(desc_sampled) != 0:
                        desc_sampled = desc_sampled.cpu().numpy()
                    #desc_sampled: an 128xnum_samples np.array.
                    collection_descriptors_2d.append(desc_sampled)
                
                #average features per sampled point
                collection_descriptors_2d = np.array(collection_descriptors_2d) #shape track.count_lines()x256xnum_samples
            ...
            #visualize reprojected sampled 3d points in images
            dummy=5
            print(cfg["output_dir"]+f"/reprojection_vis/{dummy}.png")
            os.mkdir(cfg["output_dir"]+"/reprojection_vis")
            for db_id in imagecols_db.get_img_ids():
                cv2.imwrite(cfg["output_dir"]+f"/reprojection_vis/{db_id}.png", drawn_imgs[db_id])
    ...
    for qid in tqdm(query_ids):
        ...
        for pair in all_line_pairs_2to2["direct_3d"]: #we need this because 1 query line can have matches to several 3d lines/tracks
