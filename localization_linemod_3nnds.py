...
data_folder_path=Path("/cluster/courses/3dv/data/team-22/dataset/lm/test/000005/")
...
def read_scene_lm(cfg):
    print("read_scene_lm")
    recompute_and_save_masked_imgs=True
    ...
    for filename in rgb_folder_path.iterdir():
        ...
        #pose = limap.base.CameraPose(np.linalg.inv(R_matrix), -t_matrix)
        pose = limap.base.CameraPose(R_matrix, t_matrix)

        #load image
        if recompute_and_save_masked_imgs:
            image = cv2.imread(str(filename))
            img_hw = [image.shape[0],image.shape[1]]

            mask_path = str(mask_folder_path / filename.stem) + "_000000.png"
...
def run_hloc_lm(cfg, image_dir, imagecols, neighbors, train_ids, query_ids, id_to_origin_name,
    ...
    for id in saved_train_ids: #+ saved_query_ids:
        selected = [id]
        #TODO: delete n
        visualize_sfm_2d(ref_sfm, images, color_by="visibility", selected=selected, n=5)
        save_plot(str(vis_output_path / f"hloc_reconstruction_viz_visibility_{id}.png"), dpi=1200)
        matplotlib.pyplot.close()
        visualize_sfm_2d(ref_sfm, images, color_by="track_length", selected=selected, n=5)
        save_plot(str(vis_output_path / f"hloc_reconstruction_viz_track_length_{id}.png"), dpi=1200)
        matplotlib.pyplot.close()
        visualize_sfm_2d(ref_sfm, images, color_by="depth", selected=selected, n=5)
...
def calculate_average(filtered_descinfo_folder, averagetrack_output, track_number):
    ...
    for file_path in file_list:
        ...
        # Reshape x and y values to (80, 2) so that we can average along axis 0
...
def main():
    ...
    for id in saved_train_ids: #+ saved_query_ids:
        selected = [id]
        visualize_sfm_2d(ref_sfm, images, color_by="all_same", selected=selected, n=5)
    ...
    for vis_id in range(0, num_query_imgs):
