...
# Script that reads rendered sbb door images as database images and real-life sbb door images as query images.
# Script performs point only and joint object pose detection on that data.
# However, the real-life images do not have a ground truth, so that results printed in the end are not valid., only the visualizations.
...
def parse_config():
    ...
    arg_parser.add_argument('--output_dir', type=str, default='results_sbb', help='output directory')
...
def read_scene_lm(cfg):
    ...
    #db and query have different K and maybe different img_hw so we need to add K per image and not only once
    ...
    for i, filename in enumerate(query_imgs_files):
        ...
        id_to_origin_name[short_id] = str(curr_img_path)

        #intrinsic and extrinsics parameters
        ...
        #load image
        image = cv2.imread(str(filename))
        img_hw2 = [image.shape[0],image.shape[1]]
        assert img_hw[0]==img_hw2[0], "not same image height"
        assert img_hw[1]==img_hw2[1], "not same image width"
        ...
        if i in sample:
            ...
            print(str(curr_img_path))
            camimages[short_id] = camimage

    #as db and query have different K and maybe different img_hw, we need K per image
    cameras[1] = limap.base.Camera("PINHOLE", K_query, cam_id=1, hw=img_hw)

    print("MAPPING: short_id_to_image_id")
    print(short_id_to_image_id)
    print("MAPPING FINISHED")

    # #where all the intrinsics and extrinsics of all db and query images are saved
    ...
    return imagecols, neighbors, ranges, train_ids, query_ids, id_to_origin_name, final_imgs_folder_path
...
def run_hloc_lm(cfg, image_dir, imagecols, neighbors, train_ids, query_ids, id_to_origin_name,
    ...
    #save some visualizations of point-only 3d reconstruction
    ...
    for id in saved_train_ids:
        selected = [id]
        visualize_sfm_2d(ref_sfm, image_dir, color_by="visibility", selected=selected, n=5)
        save_plot(str(vis_output_path / f"hloc_reconstruction_viz_visibility_{id}.png"), dpi=1200)
        matplotlib.pyplot.close()
        visualize_sfm_2d(ref_sfm, image_dir, color_by="track_length", selected=selected, n=5)
        save_plot(str(vis_output_path / f"hloc_reconstruction_viz_track_length_{id}.png"), dpi=1200)
        matplotlib.pyplot.close()
        visualize_sfm_2d(ref_sfm, image_dir, color_by="depth", selected=selected, n=5)
...
def main():
    ...
    logger.info("Evaluation for hloc will be done next, but evaluation not meaningful because no ground truth")
    logger.info("POSES")
    ...
    #append points to end of copied line file to get joint 3d obj with both points and lines
